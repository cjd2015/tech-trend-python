...
// USP feature: TechTrend becomes a conversational intelligence agent via Telegram
...
export class TelegramAlerter {
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    // FLASH: nuclear anomaly, or �? critical signals across domains
    if (nukeSignal) {
      return {
    ...
    // FLASH: �? critical signals AND they span multiple domains
    ...
    // PRIORITY: �? high/critical signals in same direction
    ...
    // PRIORITY: �? new OSINT posts (surge in conflict reporting)
    ...
    // ROUTINE: any critical signal OR �? high signals
  ...
  _buildEvaluationPrompt() {
    return `You are TechTrend, an elite intelligence alert evaluator for a personal OSINT monitoring system. You analyze signal deltas from a 25-source intelligence sweep and decide if the user needs to be alerted via Telegram.
...
FLASH requires: �? corroborating sources across different domains (e.g. OSINT + market data + satellite)
...
PRIORITY requires: �? signals moving in same direction, at least 1 from hard data
